...
Config File Parser - Parse and retrieve configuration by section
TSV Price Engine - Upload TSV files and query prices by SKU and time
🏗️ Architecture
...
mvn spring-boot:run
Load config (programmatically or via service): The service needs to load config first. You can do this via:
...
Run ConfigPriceEngineApplication.java
...
Architecture & Design
Clean Architecture: Clear separation between layers (Controller → Service → Parser)
TDD: Test-driven development with 42 test cases
SOLID Principles: Single Responsibility, Dependency Inversion
Design Patterns: Service Layer, DTO Pattern, Repository Pattern (in-memory)
Performance
Fast Lookups: O(1) for config sections, O(n) for price queries (n = offers per SKU)
In-Memory Storage: Optimized for read-heavy workloads
Sorted Data: Price offers sorted by start time for efficient time-based queries
Code Quality
Readable Code: Clear naming, comments, JavaDoc
Error Handling: Proper HTTP status codes, validation
Test Coverage: Comprehensive unit and integration tests
Modularity: Easy to extend and maintain
Technical Skills
Spring Boot: REST APIs, Dependency Injection, Testing
Java 17: Modern Java features
Maven: Build and dependency management
Testing: JUnit 5, Mockito, Spring Test
📚 Further Improvements
Persistence: Add database support (PostgreSQL, MongoDB)
Caching: Add Redis for distributed caching
Validation: Enhanced input validation with Bean Validation
